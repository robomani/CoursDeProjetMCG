...
static class SceneAutoLoader
{
	...
	private static void OnPlayModeChanged(PlayModeStateChange state)
	{
		...
		{
			if (EditorUtility.DisplayDialog("Scene Selection", "Select which scene to load", "Current Scene", "Master Scene"))
			{
				PreviousScene = "";
				return;
			}
		...
		if (!EditorApplication.isPlaying && !EditorApplication.isPlayingOrWillChangePlaymode && !string.IsNullOrEmpty(PreviousScene))
		{
